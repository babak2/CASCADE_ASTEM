...
//		s +=" val="+value();
	...
	{
		if(physicalPlanned == physicalMinimum) return 0;
	...
	public double value(MarketRecord m)
	{
		...
		//thisValue += market.estimateValue(i, plannedPhysicalBalance - tr.quantityTraded )
		if(physicalPlanned + quantityTraded > 0)
			//selling
	         p1= (physicalPlanned + quantityTraded)*(m.lastTradePrice *0.5);
//			p1=(physicalPlanned + quantityTraded)*(m.lastBuyOfferPrice * 0.45 + m.lastTradePrice *0.45);
		else
			//BUying
			p1 = (physicalPlanned + quantityTraded)*(m.lastTradePrice *1.5);
//			p1 = (physicalPlanned + quantityTraded)*(m.lastSellOfferPrice * 0.55 + m.lastTradePrice *0.55);
		//System.out.print(","+p1);
	...
	{
		  //Optomisez this single record with no respect to the other steps in the plan
	      ...
	      for(int i = 0; i <10; i++)
	      {
		      ...
		      do
		      {
			      ...
			      if(lower.physicalPlanned < lower.mPCurve.minimumBalance) lower.physicalPlanned = lower.mPCurve.minimumBalance;
			      if(upper.physicalPlanned > upper.mPCurve.maximumBalance) upper.physicalPlanned = upper.mPCurve.maximumBalance;
			      
			      double valueGrad= (upper.value(m) - lower.value(m))
			    		            / (upper.physicalPlanned - lower.physicalPlanned);
			      
			      //Move the physical planed to increase value
			      physicalPlanned += valueGrad;
			      
				  //Check that the physical planned is still in bounds
			      if(physicalPlanned < mPCurve.minimumBalance)physicalPlanned = mPCurve.minimumBalance;
			      if(physicalPlanned > mPCurve.maximumBalance)physicalPlanned = mPCurve.maximumBalance;
			      //System.out.println("["+(upper.physicalPlanned - lower.physicalPlanned)+"]"+physicalPlanned);
